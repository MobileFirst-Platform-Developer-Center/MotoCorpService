...
After integrating your ui with your backend with an easily implementable security framework, you may face your next hurdle - firewalls.
...
IBM Bluemix Secure Gateway service allows developers to securely and easily tunnel through these firealls so that your end users can access the rsources they need.
...
On this Lab we will a CRM Mockup(See: :pushpin:  **Link to CRM Setup** ) that is running in a nodeJS server that is not in bluemix environment and not exposed to the web.
...
2 - To protect the CRM API to be only visible by the Mobile Foundation Runtime running on(restricted access)
...
> **Before Proceding:**   This tutorial will require you have a service to be exposed in a tcp port, then expose expose it via Secure Gateway Bluemix service. To help you on that we provide the CRM Mockup as a sample of service to be exposed.
...
> The gateway is a collection of destinations, it has secure token that allow it to be customized by the users.
...
>
>  Secure Gateway is a unidirectional bridge between 2 networks, to allow it to work properly you need a client that is "OnPrem" network to allow the Bluemix environment to reach it. So, for this example we will have a endpoint on bluemix that will receive the requests  and tunnel it to the Secure Gateway client and call the resource on the OnPrem network.

  ![Demo Map](/Lab/img/SGW_Client.png)
> The Secure Gateway client and the service/resource you want to reach on the OnPrem network do not need to be on the same machine, as far the client machine can reach the service desired it will do the proper routing to it.
...
>
> It is a endpoint on bluemix that will receive the calls from the cloud-side(Bluemix) and tunnel it in to your OnPrem resource, for example the CRM Mockup.
...
>
> The ACL it is a local firewall on the client side of the tunnel, in this case the OnPrem side, that allow you to create rules to allow or block some server addresses and ports.
&nbsp;
...
Congratulations we now have a OnPrem Service exposed to the web, allowing also it to be reached by Bluemix Applications. On the next topic we will cover how to to restrict the access to this service, for just bluemix apps.
...
> :memo:  What the SecureGatewayAdapter does?
>
> It will be used as filter to requests to your CRM to ensure the MFP Server you are using it is added on the Restricted Access list from step 1, then allowing it access to your protected resource in a given destination_id.
