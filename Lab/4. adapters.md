...
To incorporate the Swagger API into your adapter, you will need to import the configAPI so that you can change where your adapter can point to for the CRM. You will be able to change this configuration in the adapter.xml like `<property name="onPremCRMAddress" defaultValue="<host>:port/customers/"/>` or the MobileFirst operations xml.
