...
public class MessageHubConsumer {
    ...
    private static KafkaConsumer<byte[], byte[]> producer = null;

    public static KafkaConsumer<byte[], byte[]> getInstance(MessageHubProperties properties) {

        if (producer == null) {
            // Create a Kafka producer, providing client configuration.
            producer = new KafkaConsumer<byte[], byte[]>(properties.getConfig());
        }
